...
std::unordered_map<PakHandle_t, std::vector<std::unique_ptr<CDynamicImageAtlas>>>& CDynamicImageAtlas::s_AtlasesByPak =
...
std::unordered_map<CDynamicImageAtlas::Handle, std::unique_ptr<CDynamicImageAtlas>>& CDynamicImageAtlas::s_ScriptAtlases =
...
DECLARE_HOOK(Rui_FindImageAsset, engine.dll + 0xF8000, [](auto& hook, RuiInstance* rui, const char* imagePath) -> RuiImageHandle
{
    CDynamicImageAtlas::TryRegisterImage(imagePath);
    CImageAtlas::RequestNativeMenuImage(imagePath);
    return hook.Original(rui, imagePath);
})

bool CDynamicImageAtlas::ParseImage(const rapidjson_document::ValueType& value, Image& image, std::string& errorMessage)
{
    if (!value.IsObject() || !value.HasMember("path") || !value["path"].IsString() || value["path"].GetStringLength() == 0)
    {
    ...
    if (!readUint("posX", image.m_PosX) || !readUint("posY", image.m_PosY) || !readUint("width", image.m_Width) ||
        !readUint("height", image.m_Height))
    {
        return false;
    }
...
std::unique_ptr<CDynamicImageAtlas> CDynamicImageAtlas::ParseDefinition(const rapidjson_document::ValueType& value, const fs::path& sourcePath)
{
    if (!value.IsObject())
    {
    ...
    if (!value.HasMember("textures"))
    {
        ...
        return std::unique_ptr<CDynamicImageAtlas>(new CDynamicImageAtlas(sourcePath, std::move(texturePath), std::nullopt, std::move(images)));
    }

    if (!value["textures"].IsArray() || value["textures"].Empty())
    {
        spdlog::error("RUI atlas '{}': 'textures' must be a non-empty array", sourcePath.string());
        return nullptr;
    }

    if (value["textures"].Size() > static_cast<rapidjson::SizeType>(INT16_MAX))
    {
        spdlog::error("RUI atlas '{}': atlas '{}' contains too many images (maximum {})", sourcePath.string(), texturePath, INT16_MAX);
        return nullptr;
    }
    ...
    for (const auto& imageValue : value["textures"].GetArray())
    {
        ...
        {
            spdlog::error("RUI atlas sidecar '{}': image '{}' collides with another image name in the same atlas", sourcePath.string(), image.m_Path);
            return nullptr;
        }
...
std::vector<std::unique_ptr<CDynamicImageAtlas>> CDynamicImageAtlas::ParseSidecar(const fs::path& sourcePath)
{
    ...
    {
        spdlog::error("Failed parsing RUI atlas '{}': {} at byte {}", sourcePath.string(),
    ...
    else if (document.IsObject() && document.HasMember("atlases"))
    {
        ...
        {
            spdlog::error("RUI atlas '{}': 'atlases' must be an array", sourcePath.string());
            return {};
        }
    ...
    {
        spdlog::error("RUI atlas '{}': root must be an atlas object or array", sourcePath.string());
    }

    return definitions;
}

std::unique_ptr<CDynamicImageAtlas> CDynamicImageAtlas::ParseScript(uint64_t textureGuid, const char* jsonData, std::string& errorMessage)
{
    ...
    {
        errorMessage =
            fmt::format("invalid atlas JSON: {} at byte {}", rapidjson::GetParseError_En(document.GetParseError()), document.GetErrorOffset());
        return nullptr;
    }
    ...
    return std::unique_ptr<CDynamicImageAtlas>(
        new CDynamicImageAtlas("NS_CreateImageAtlas", fmt::format("GUID 0x{:016X}", textureGuid), textureGuid, std::move(images)));
}
...
CDynamicImageAtlas::CDynamicImageAtlas(fs::path sourcePath, std::string texturePath, std::optional<uint64_t> textureGuid, std::vector<Image> images)
    : m_SourcePath(std::move(sourcePath)), m_TexturePath(std::move(texturePath)), m_TextureGuid(textureGuid), m_Images(std::move(images))
{
}
...
bool CDynamicImageAtlas::MakeResident()
{
    ...
    {
        spdlog::error("Cannot create runtime RUI atlas from '{}': the engine atlas loader is not ready", m_SourcePath.string());
        return false;
    }

    const uint64_t textureGuid = m_TextureGuid ? *m_TextureGuid : CImageAtlas::HashAssetPath(m_TexturePath.c_str());
    ...
    auto* texture = reinterpret_cast<TextureAsset_s*>(g_pakLoadApi->GetAssetBinding(textureGuid));
    if (!texture)
    {
        spdlog::error("RUI atlas '{}': TXTR '{}' (GUID 0x{:016X}) is not loaded", m_SourcePath.string(), m_TexturePath, textureGuid);
        return false;
    }
    ...
    {
        spdlog::error("RUI atlas '{}': TXTR '{}' has invalid dimensions {}x{}", m_SourcePath.string(), m_TexturePath, atlasWidth,
                      atlasHeight);
        return false;
    }
    ...
    for (Image& image : resolvedImages)
    {
        ...
        {
            spdlog::error("RUI atlas '{}': image '{}' bounds ({}, {}, {}, {}) exceed texture '{}' dimensions {}x{}", m_SourcePath.string(),
                          image.m_Path, image.m_PosX, image.m_PosY, image.m_Width, image.m_Height, m_TexturePath, atlasWidth, atlasHeight);
            m_Invalid = true;
            return false;
        }
    }
    ...
    {
        atlasImages.push_back({image.m_NameHash, image.m_PosX, image.m_PosY, image.m_Width, image.m_Height, image.m_Flags});
    }

    if (!m_ImageAtlas.Create(static_cast<uint16_t>(atlasWidth), static_cast<uint16_t>(atlasHeight), texture, atlasImages))
    {
        spdlog::error("Failed creating runtime RUI atlas '{}' (no slot, GPU buffer, or descriptor available)", m_TexturePath);
        return false;
    }
    ...
    spdlog::info("Registered texture '{}' as runtime RUI atlas {} with {} image(s)", m_TexturePath, *atlasIndex, m_Images.size());
    return true;
}
...
bool CDynamicImageAtlas::TryRegisterImage(const char* imagePath)
{
    ...
    spdlog::info("Runtime RUI image lookup '{}' matched hash 0x{:08X}", imagePath, nameHash);
    ...
    spdlog::error("Runtime RUI image '{}' matched but descriptor registration failed for hash 0x{:08X}", imagePath, nameHash);
    return false;
}

std::optional<CDynamicImageAtlas::Handle> CDynamicImageAtlas::Create(uint64_t textureGuid, const char* jsonData, std::string& errorMessage)
{
    ...
    for (const Image& image : definition->m_Images)
    {
        ...
        {
            errorMessage =
                fmt::format("cannot register RUI image '{}' because hash 0x{:08X} is already registered or reserved", image.m_Path, image.m_NameHash);
            return std::nullopt;
        }
    }
    ...
    {
        errorMessage = fmt::format("could not create an atlas for loaded TXTR GUID 0x{:016X}; check the log for the exact failure", textureGuid);
        return std::nullopt;
    }
...
void CDynamicImageAtlas::OnPakLoaded(const fs::path& pakPath, PakHandle_t handle)
{
    ...
    spdlog::info("Loaded {} runtime RUI atlas definition(s) from '{}'", definitions.size(), sidecarPath->string());
...
ON_DLL_LOAD("engine.dll", RuiDynamicImageAtlas, [](CModule) { DISPATCH_MODULE(RuiDynamicImageAtlasHooks); })
