...
RemoteServerInfo::RemoteServerInfo(const char* newId, const char* newName, const char* newDescription, const char* newMap, const char* newPlaylist,
                                   const char* newRegion, int newPlayerCount, int newMaxPlayers, bool newRequiresPassword)
{
...
void MasterServerManager::AuthenticateOriginWithMasterServer()
{
    ...
    std::thread requestThread([this]()
    {
        ...
        {
            if (std::optional<std::string> newToken = GetNewOriginToken(std::chrono::seconds(5)))
                tokenStr = std::move(*newToken);
            ...
            if (result == CURLcode::CURLE_OK)
            {
                ...
                if (originAuthInfo.HasParseError())
                {
                    ...
                    spdlog::error("Failed reading origin auth info response: encountered parse error \"{}\"",
            ...
            {
                spdlog::error("Failed performing northstar origin auth: error {} (attempt {}/{})", curl_easy_strerror(result), attempt + 1,
                              maxAttempts);
...
void MasterServerManager::RequestServerList()
{
    ...
    std::thread requestThread([this]()
    {
        ...
        if (result == CURLcode::CURLE_OK)
        {
            ...
            {
                spdlog::error("Failed reading masterserver response: encountered parse error \"{}\"",
            ...
            for (auto& serverObj : serverArray)
            {
                ...
                if (!serverObj.HasMember("id") || !serverObj["id"].IsString() || !serverObj.HasMember("name") || !serverObj["name"].IsString() ||
                    !serverObj.HasMember("description") || !serverObj["description"].IsString() || !serverObj.HasMember("map") ||
                    !serverObj["map"].IsString() || !serverObj.HasMember("playlist") || !serverObj["playlist"].IsString() ||
                ...
                for (RemoteServerInfo& server : m_vRemoteServers)
                {
                    ...
                    {
                        server =
                            RemoteServerInfo(id, serverObj["name"].GetString(), serverObj["description"].GetString(), serverObj["map"].GetString(),
                                             serverObj["playlist"].GetString(),
                                             (serverObj.HasMember("region") && serverObj["region"].IsString()) ? serverObj["region"].GetString() : "",
                                             serverObj["playerCount"].GetInt(), serverObj["maxPlayers"].GetInt(), serverObj["hasPassword"].IsTrue());
                        newServer = &server;
            ...
            std::sort(m_vRemoteServers.begin(), m_vRemoteServers.end(),
...
void MasterServerManager::RequestMainMenuPromos()
{
    ...
    std::thread requestThread([this]()
    {
        ...
        if (result == CURLcode::CURLE_OK)
        {
            ...
            {
                spdlog::error("Failed reading masterserver main menu promos response: encountered parse error \"{}\"",
...
void MasterServerManager::AuthenticateWithOwnServer(const char* uid, const char* playerToken, netadr_t addr)
{
    ...
    std::thread requestThread([this, uidStr, tokenStr, addr]()
    {
        CURL* curl = curl_easy_init();
        SetCommonHttpClientOptions(curl);
        ...
        curl_easy_setopt(
            curl, CURLOPT_URL,
            fmt::format("{}/client/auth_with_self?id={}&playerToken={}", Cvar_ns_masterserver_hostname->GetString(), uidStr, tokenStr).c_str());
        ...
        if (result == CURLcode::CURLE_OK)
        {
            ...
            {
                spdlog::error("Failed reading masterserver authentication response: encountered parse error \"{}\"",
            ...
            if (!authInfoJson.HasMember("success") || !authInfoJson.HasMember("id") || !authInfoJson["id"].IsString() ||
                !authInfoJson.HasMember("authToken") || !authInfoJson["authToken"].IsString() || !authInfoJson.HasMember("persistentData") ||
            ...
            if (addr.GetType() == NA_IP)
            {
                ...
                NET_SendPacket(nullptr, NS_SERVER, &addr, notifyWriteBuffer.GetData(), notifyWriteBuffer.GetNumBytesWritten(), nullptr, false, 0,
                               true);
...
void MasterServerManager::AuthenticateWithServer(const char* uid, const char* playerToken, RemoteServerInfo server, const char* password)
{
    ...
    std::thread requestThread([this, uidStr, tokenStr, serverIdStr, passwordStr, server]()
    {
...

        {
            ...
            curl_easy_setopt(curl, CURLOPT_URL,
                             fmt::format("{}/client/auth_with_server?id={}&playerToken={}&server={}&password={}",
                                         Cvar_ns_masterserver_hostname->GetString(), uidStr, tokenStr, serverIdStr,
        ...
        if (result == CURLcode::CURLE_OK)
        {
            ...
            if (!connectionInfoJson.HasMember("success") || !connectionInfoJson.HasMember("ip") || !connectionInfoJson["ip"].IsString() ||
                !connectionInfoJson.HasMember("port") || !connectionInfoJson["port"].IsNumber() || !connectionInfoJson.HasMember("authToken") ||
            ...
            strncpy_s(m_pendingConnectionInfo.authToken, sizeof(m_pendingConnectionInfo.authToken), connectionInfoJson["authToken"].GetString(),
...
void MasterServerManager::WritePlayerPersistentData(const char* playerId, const char* pdata, size_t pdataSize)
{
    ...
    std::thread requestThread([this, strPlayerId, strPdata, pdataSize]
    {
        ...
        curl_easy_setopt(
            curl, CURLOPT_URL,
            fmt::format("{}/accounts/write_persistence?id={}&serverId={}", Cvar_ns_masterserver_hostname->GetString(), strPlayerId, m_sOwnServerId)
                .c_str());
...
void MasterServerManager::ProcessConnectionlessPacketSigreq1(std::string data)
{
    ...
    if (type == "connect")
    {
        ...
        if (reject == "")
        {
            ...
            curl_easy_setopt(
                curl, CURLOPT_URL,
                fmt::format("{}/server/connect?serverId={}&token={}", Cvar_ns_masterserver_hostname->GetString(), m_sOwnServerId, token).c_str());
            ...
            if (respStatus != 200)
            {
                ...
                if (!obj.HasParseError() && obj.HasMember("error") && obj["error"].IsObject())
                    spdlog::error("failed to make Atlas connect pdata request {}: response status {}, error: {} ({})", token, respStatus,
            ...
            {
                spdlog::error("failed to make Atlas connect pdata request {}: pdata is too large (max={} len={})", token, PERSISTENCE_MAX_SIZE,
                              pdata.length());
                return;
            }
        }
...

        {
            ...
            curl_easy_setopt(curl, CURLOPT_URL,
                             fmt::format("{}/server/connect?serverId={}&token={}&reject={}", Cvar_ns_masterserver_hostname->GetString(),
                                         m_sOwnServerId, token, rejectEnc)
                                 .c_str());
            curl_free(rejectEnc);
            ...
            if (respStatus != 200)
            {
                ...
                if (!obj.HasParseError() && obj.HasMember("error") && obj["error"].IsObject())
                    spdlog::error("failed to respond to Atlas connect request {}: response status {}, error: {} ({})", token, respStatus,
                                  ((obj["error"].HasMember("enum") && obj["error"]["enum"].IsString()) ? obj["error"]["enum"].GetString() : ""),
                                  ((obj["error"].HasMember("msg") && obj["error"]["msg"].IsString()) ? obj["error"]["msg"].GetString() : ""));
                else
...
void ConCommand_ns_try_join_serverid(const CCommand& args)
{
    ...
    std::thread t1([serverId, password]
    {
        ...
        g_pMasterServerManager->AuthenticateWithServer(g_pLocalPlayerUserID, g_pMasterServerManager->m_sOwnClientAuthToken,
                                                       g_pMasterServerManager->m_vRemoteServers[serverIndex], (char*)password.c_str());
        ...
        Cbuf_AddText(Cbuf_GetCurrentPlayer(),
                     fmt::format("connect {}.{}.{}.{}:{}", info.ip.S_un.S_un_b.s_b1, info.ip.S_un.S_un_b.s_b2, info.ip.S_un.S_un_b.s_b3,
...
void MasterServerPresenceReporter::DestroyPresence(const ServerPresence* pServerPresence)
{
    ...
    *g_pMasterServerManager->m_sOwnServerId = 0;
    ...
    std::thread requestThread([this, url]
    {
        CURL* curl = curl_easy_init();
        SetCommonHttpClientOptions(curl);
...
void MasterServerPresenceReporter::RunFrame(double flCurrentTime, const ServerPresence* pServerPresence)
{
    ...
    if (addServerFuture.valid())
    {
        ...
        case MasterServerReportPresenceResult::Success:
            ...
            strncpy_s(g_pMasterServerManager->m_sOwnServerId, sizeof(g_pMasterServerManager->m_sOwnServerId), resultData.id.value().c_str(),
                      sizeof(g_pMasterServerManager->m_sOwnServerId) - 1);
            strncpy_s(g_pMasterServerManager->m_sOwnServerAuthToken, sizeof(g_pMasterServerManager->m_sOwnServerAuthToken),
                      resultData.serverAuthToken.value().c_str(), sizeof(g_pMasterServerManager->m_sOwnServerAuthToken) - 1);
            break;
        ...
        if (m_nNumRegistrationAttempts >= MAX_REGISTRATION_ATTEMPTS)
        {
            spdlog::log(IsDedicatedServer() ? spdlog::level::level_enum::err : spdlog::level::level_enum::warn,
...
void MasterServerPresenceReporter::InternalAddServer(const ServerPresence* pServerPresence)
{
    ...
    addServerFuture = std::async(std::launch::async, [threadedPresence, modInfo, hostname, pServerPresence]
    {
        CURL* curl = curl_easy_init();
        SetCommonHttpClientOptions(curl);
        ...
        bool shouldLogError =
            IsDedicatedServer() || (!strstr(pServerPresence->m_MapName, "mp_lobby") && strstr(pServerPresence->m_PlaylistName, "private_match"));
        ...
        // format every paramter because computers hate me
        {
            ...
            curl_easy_setopt(curl, CURLOPT_URL,
                             fmt::format("{}/server/"
                                         ...
                                         hostname.c_str(), threadedPresence.m_iPort, nameEscaped, descEscaped, mapEscaped, playlistEscaped,
                                         threadedPresence.m_iMaxPlayers, passwordEscaped)
                                 .c_str());
        ...
        if (result == CURLcode::CURLE_OK)
        {
            ...
            return ReturnCleanup(MasterServerReportPresenceResult::Success, serverAddedJson["id"].GetString(),
        ...
        // without a restart this isn't threadsafe :terror:
        {
            ...
            curl_easy_setopt(curl, CURLOPT_URL,
                             fmt::format("{}/server/"
                                         ...
                                         "maxPlayers={}&password={}",
                                         hostname.c_str(), serverId.c_str(), threadedPresence.m_iPort, nameEscaped, descEscaped, mapEscaped,
                                         playlistEscaped, threadedPresence.m_iPlayerCount, threadedPresence.m_iMaxPlayers, passwordEscaped)
                                 .c_str());
